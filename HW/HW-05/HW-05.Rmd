...
model_nnet <- train.ts_diff %>%
  model(hand1 = NNETAR(diff_mw ~ AR(p = 2, P = 3), n_nodes = 5))
...
# fc on test
ggplot(test.ts, aes(x = datetime_beginning_ept)) +
  geom_line(aes(y = mw, color = "Actual"), show.legend = TRUE, size = 1.0) +
  geom_line(aes(y = nnet_for_hand, color = "Predicted"), show.legend = TRUE, size = 1.0) +
  labs(y = "Hourly Megawatt (MW)", x = "Date", title = "Actual versus Forecast Values for Neural Network Model", color = NULL)
```
